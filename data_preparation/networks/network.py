...
class PRNet(nn.Module):
    def __init__(self, in_channels, out_channels=3):
        ...
        self.input_conv = nn.Sequential( #*[
            *padding_same_conv2d(256, in_channels, size, kernel_size=4, stride=1),  # 256x256x16
        ...
        self.output_conv = nn.Sequential(
            nn.Conv2d(size, out_channels, 1, stride=1, padding=0),

            #nn.ConstantPad2d((2, 1, 2, 1), 0),
            #nn.ConvTranspose2d(size, out_channels, kernel_size=4, stride=1, padding=3, bias=False),
...
class PRNet_slim(nn.Module):
    def __init__(self, in_channels, out_channels=3):
        ...
        self.up_conv_5 = upBlock(size * 32, size * 16, conv_num=0)  # 16x16x256
        self.up_conv_4 = upBlock(size * 16, size * 8, conv_num=0)  # 32x32x128
        self.up_conv_3 = upBlock(size * 8, size * 4, conv_num=0)  # 64x64x64

        self.up_conv_2 = upBlock(size * 4, size * 2, conv_num=0)  # 128x128x32
        self.up_conv_1 = upBlock(size * 2, size, conv_num=0)  # 256x256x16
