function [facial_amb_dir_ctex_img, facial_amb_dir_ctex_uv , facial_itex_img, facial_amb_ctex_img, facial_amb_ctex_uv, facial_dir_itex_img , alpha_harmonic, alpha_harmonic_amb, alphas_tex, alphas_tex_amb, common_tex, common_tex_amb] = FacialTrendFitting(vertex, tri, mu_tex, w_tex, sigma_tex, norm, valid_bin, img, UV, facial_detail_distribute)
...
[facial_amb_dir_ctex_img, valid_map] = Mex_ZBuffer(vertex_img, tri, facial_amb_dir_ctex, img);
facial_itex_img = img - facial_amb_dir_ctex_img;

temp = zeros(height, width, nChannels);
temp(:,:,1) = valid_map; temp(:,:,2) = valid_map; temp(:,:,3) = valid_map;
valid_map = temp(:);
invalid_bin = valid_map==0;


% for c = 1:nChannels
%     temp = facial_itex_img(:,:,c);
%     %temp = temp - facial_itex_min(c) / (facial_itex_max(c) - facial_itex_min(c));
%     temp = temp - facial_itex_min / (facial_itex_max - facial_itex_min);
%     facial_itex_img(:,:,c) = temp;
% end
facial_itex_img = facial_itex_img - facial_itex_min / (facial_itex_max - facial_itex_min);
facial_itex_img(invalid_bin) = 0;
...
facial_dir_itex_img = (facial_dir_itex_img - facial_dir_itex_min) / (facial_dir_itex_max - facial_dir_itex_min);
% for c = 1:nChannels
%     temp = facial_dir_itex_img(:,:,c);
%     %temp = temp - facial_dir_itex_min(c) / (facial_dir_itex_max(c) - facial_dir_itex_min(c));
%     temp = temp - facial_dir_itex_min / (facial_dir_itex_max - facial_dir_itex_min);
%     facial_dir_itex_img(:,:,c) = temp;
% end
facial_dir_itex_img(invalid_bin) = 0;
